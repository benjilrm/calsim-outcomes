...
process_file = function(file_path, crosswalk, data_type = c("demand", "delivery")) {
  ...
  processed = data%>%
    ...
    group_by(demand_unit_id)%>%
    mutate(sum = sum(value_taf))%>%
    filter(sum != 0)%>%
    select(-sum)
...
process_scenario = function(scenario_id, demand_folder, delivery_folder, crosswalk, output_base = "Outputs") {
  ...
  tiers = combined_by_year %>%
    mutate(demand_met = ifelse(percent_demand_met >= 90, 1, 0)) %>%
  ...
  return(list(tiers = tiers, missing = missing))}
```
...
process_all_scenarios = function(demand_folder, delivery_folder, crosswalk, output_base = "Outputs") {
  ...
  all_missing = map_df(all_results, ~ .x$missing)
  ...
  if (!is.null(output_base) && nrow(all_missing) > 0) {
    missing_dir = file.path(output_base, "Missing demand units")
    dir.create(missing_dir, showWarnings = F, recursive = T)
    write.csv(all_missing, file.path(missing_dir, "all_scenarios_missing_dus.csv"), row.names = F)}
  
  # Return both for further analysis if needed
  return(list(tier_matrix = tier_matrix, missing_data = all_missing))}
```
