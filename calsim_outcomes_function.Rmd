...
process_file = function(file_path, crosswalk, data_type = c("demand", "delivery")) {
  ...
  not_present = crosswalk %>%
    ...
    distinct() %>%
    mutate(status = ifelse(!.data[[id_col]] %in% processed[[id_col]], 
                           "Not in data", NA)) %>%
    filter(!is.na(status))
  ...
  all_blank_or_all_zero = processed %>%
    ...
    reframe(
      status = case_when(
        all(is.na(.data[[value_col]])) ~ "All blank", 
        all(.data[[value_col]] == 0, na.rm = T) ~ "All zero", 
        T ~ NA)) %>%
    ungroup() %>%
    filter(!is.na(status))
...
process_all_scenarios = function(demand_folder, delivery_folder, crosswalk, output_base = "Outputs") {
  ...
  if (!is.null(output_base)) {
    ...
    write.csv(tier_matrix, file.path(tier_dir, "20260227_historical_scenarios_tiers.csv"), row.names = F)}
...
read.csv("Outputs/Monthly demand met/s0011_demand_met_by_month.csv")%>%
  filter(dwuc == "MWD")%>%
  mutate(date = as.Date(date))%>%
  ggplot(aes(x = date, y = delivery_taf))+
  geom_line(color = "steelblue", size = 0.5) +
  #geom_smooth(method = "loess", color = "red", se = TRUE, alpha = 0.2) +
  scale_x_date(date_breaks = "10 years", 
               date_labels = "%Y",
               limits = as.Date(c("1921-10-31", "2021-09-30"))) +
  labs(title = "Monthly Time Series, Metropolitan",
       x = "Year", 
       y = "Monthly Delivery (TAF)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
  reframe(mean(annual_delivery))
