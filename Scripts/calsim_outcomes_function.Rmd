...
# Demand data
```{r}
static_demand = read.csv("Outputs/Static demand/20260410_static_demand_by_du_updated.csv")%>%
...
process_all_scenarios = function(delivery_folder, crosswalk, output_base = "Outputs") {
  ...
  write.csv(tier_matrix_categorical, file.path(tier_dir, "20260410_all_scenarios_through_cc95_tiers_categorical.csv"), row.names = F)
  write.csv(tier_matrix_continuous, file.path(tier_dir, "20260410_all_scenarios_through_cc95_tiers_continuous.csv"), row.names = F)}
```
...
Plot for baseline
```{r}
read.csv("Outputs/Tier summaries/20260410_all_scenarios_through_cc95_tiers_categorical.csv")%>%
