...
crosswalk = read_excel("Inputs/Master crosswalk SW DUs M&I.xlsx")%>%
  ...
  mutate(dwuc = str_replace_all(dwuc, "[\n]+", ""),
         delivery_id = case_when(
           ...
           T ~ delivery_id))%>%
  filter(!dwuc %in% c("71_NU", "72_NU", "26S_NU3", "ESB415"))

write.csv(crosswalk, "Outputs/Crosswalk demand and delivery IDs/20260407_crosswalk_processed.csv", row.names = T)
```
...
static_demand = read.csv("Outputs/Static demand/20260407_static_demand_by_du.csv")%>%
  ...
  mutate(annual_demand = case_when(
    year == 1921 ~ annual_demand*0.75,
    year == 2021 ~ annual_demand*0.25,
    T ~ annual_demand))
```
