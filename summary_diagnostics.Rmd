...
read.csv("Outputs/Tier summaries/all_scenarios_tier_matrix_alt5.csv")%>%
  ...
  labs(x= "Scenario", y = "Percentage of Demand Units", fill = "Tier", caption = "Insert tier definitions here")+
  theme_bw()
...
read.csv("Outputs/Tier summaries/all_scenarios_tier_matrix_alt5.csv")%>%
  ...
  ggplot(aes(scenario_id, prop*100, fill = as.factor(tier)))+
  geom_col(position="dodge")+
  labs(x= "Baseline Scenario", y = "Percentage of Demand Units", fill = "Tier")+
  theme_bw()
```
```{r}
...
dwucs_to_keep <- c("20_NU1", "NAPA", "61_NU2", "90_PU", "MWD")
...
all_data <- map_dfr(scenarios, function(scenario) {
  ...
  df <- read.csv(file_path)%>%
      ...
      reframe(monthly_avg = median(delivery_taf),
              p10 = quantile(delivery_taf, 0.1),
              p90 = quantile(delivery_taf, 0.9))
  return(df)})

ggplot(all_data, aes(x = month, y = monthly_avg)) +
  geom_point(color = "steelblue")+
  geom_line(color = "steelblue", linewidth = 0.7) +
  geom_point(color = "steelblue", size = 0.5, alpha = 0.3) +
  facet_wrap(dwuc ~ scenario_id,  ncol = 4, nrow = 5) +
